...
def log(filename='logging.txt'):
	...
	with open(filename) as f:
		...
		for line in lines:
			...
			auc.append(float(line[3].split(':')[-1].strip()))
			# prc.append(float(line[4].split(':')[-1].strip()))
				

	data = pd.DataFrame()
	data['pcc'], data['spcc'], data['mcc'], data['auc'] = pcc, spcc, mcc, auc
	# data['pcc'], data['spcc'], data['mcc'], data['auc'], data['prc'] = pcc, spcc, mcc, auc, prc
