...
from load_data import get_dataloader_rt_or_inter, get_dataloader_div_by_gene, get_dataloader_virus_inter
from myutils import *

modules_attn = __import__('model_attention')
...
parser.add_argument('--sample_internal', type=int, default=10, help='the num of k_cross testation division')
...
class K_Train_Test():
    ...
    def model_train(self, train_loader, test_loader, testset_type):
        ...
        model_name = 'model_att_' + testset_type.replace('_', '') + '.pth.tar'
        save_path = os.path.join('./output/attention_2_mse_loss', str(testset_type))
        ...
        for epoch in range(self.args.n_epochs):
            ...
            early_stopping(PCC, epoch, model)
            if early_stopping.early_stop:
                break


        # 测试early_Stopping中保存过的最好的模型，用来绘图
        ...
        with torch.no_grad():
            ...
            logging('epoch {}  ====>> pcc:{:.4f}, spcc:{:.4f}, mcc:{:.4f}, auc:{:.4f}'.format(
                epoch, PCC, SPCC, MCC, AUC))
            print('epoch {}  ====>> pcc:{:.4f}, spcc:{:.4f}, mcc:{:.4f}, auc:{:.4f}'.format(
                epoch, PCC, SPCC, MCC, AUC))
    ...
    def k_cross(self):
        ...
        for k in range(k_fold):
            inter_type = str(k)+'-inter'
            train_loader, test_loader = get_dataloader_rt_or_inter(self.args, inter_type)
            self.model_train(train_loader, test_loader, inter_type)
